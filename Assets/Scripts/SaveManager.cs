...
public class SaveManager : MonoBehaviour
{
    ...
    public void SaveGame(GameManager gm, TimeManager tm, ProgressionManager pm)
    {
        ...
        if (om != null && om.activeOrders != null)
        {
            ...
            var counterField = om.GetType().GetField("orderCounter",
    ...
    public void ResetGame()
    {
        ...
        // Réinitialise les managers spécifiques
        EmployeeManager employeeManager = EmployeeManager.Instance;
        if (employeeManager != null)
        {
            employeeManager.ResetEmployees();
        }
        
        BuildingManager buildingManager = BuildingManager.Instance;
        if (buildingManager != null)
        {
            // Réinitialise les données de construction
            for (int i = 0; i < buildingManager.allElements.Count; i++)
            {
                buildingManager.allElements[i].isPurchased = false;
                
                // L'atelier de base (index 0) reste acheté
                if (i == 0)
                {
                    buildingManager.allElements[i].isPurchased = true;
                }
            }
        }
        ...
        if (orderManager != null && orderManager.activeOrders != null)
        {
        ...
        {
            statsManager.stats = new StatsData();
        }
